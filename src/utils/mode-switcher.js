...
export class ModeSwitcher {
  ...
  /**
   * Fallback order when the viewer has never toggled:
   *   1. the mode the ROUTE painted on <html data-mode> (the author's intent
   *      for this direction -- journal defaults light, atlas defaults dark)
   *   2. the OS preference
   * Only an explicit toggle writes to localStorage, so an author default is
   * never sticky and never overrides a real choice.
   */
  getSystemPreference() {
    if (typeof document !== 'undefined') {
      const painted = document.documentElement.getAttribute('data-mode');
      if (VALID_MODES.includes(painted)) return painted;
    }
