...
// - Single-brand sites: leave as ['journal'] (toggle becomes no-op).
// - Multi-brand sites: add brand keys, e.g. ['journal', 'water', 'matter'].
// ============================================================================

const VALID_THEMES = ['journal', 'atlas', 'practice'];

export class ThemeSwitcher {
  constructor() {
    this.currentTheme = this.getStoredTheme() || 'journal';
  ...
  getStoredTheme() {
    ...
    return VALID_THEMES.includes(stored) ? stored : 'journal';
  }
  ...
  applyTheme(theme, initialLoad = false) {
    ...
    const next = VALID_THEMES.includes(theme) ? theme : 'journal';
  ...
  getCurrentTheme() {
    ...
    const html = document.documentElement;
    return VALID_THEMES.find((t) => html.classList.contains(`theme-${t}`)) || 'journal';
  }
